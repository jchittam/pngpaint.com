...
(function($) {
    ...
    prty = {
        ...
        Setup: function(gDiv, settings) {
            ...
            prty.galleryDiv = gDiv.append('<ul style="padding-left:0px;" class="entries-columns"></ul>').find('ul');
            flickr = new Flickr({
        ...
        buildThumb: function() {
            ...
            for (var i = 0; i < prty.photoDetails.length; i++) {
                ...
                thisItem = prty.galleryDiv.append("<li id=" + i + "><a style='z-index:-9999;' href='" + img.url_o + "'><img src='" + img.url_m + "' alt='" + img.title + "' /><div class='bg'></div><div class='content'></div><i class='show' data-href='" + img.url_o + "' data-photoid='" + img.id + "'>VIEW</i></a></li>").find('li .content').last();
        ...
        setDetails: function(settings) {
            ...
            flickr.photos.getInfo({
                ...
                if (!err) {
                    ...
                    //Push the details
                    prty.vbDetails.contents().remove().addBack().append('<h2>' + photo.title._content + '</h2><p>' + photo.description._content + '</p><div class="date"><span>' + pDate.day + ' ' + pDate.month + ', ' + pDate.year + '</span></div><div class="author" style="display:none;">Published by: ' + photo.owner.realname + '</div>');
